...
> ## ✅ СТАН 2026-06-28: УСІ БЛОКЕРИ ВИКОНАНО власником на проді + smoke-тест пройдено.
> Деталі стану й наступний беклог — у `WORKLOG.md` → блок **🚦 START HERE**.
> Свідомо відкладено власником: `config.legal`/`payment` (пізніше), Cloudflare WAF (платна, нема доступу), `MANAGER_PASSWORD` (опц.).
> Нижче — історичний чеклист (для довідки / повторного деплою).
